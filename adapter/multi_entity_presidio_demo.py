...
os.environ["GEMINI_API_KEY"] = "AIzaSyBR9IyCRmhw-n4Wax_LcBppKkCeSgvdq3s"
...
async def proxy_chat(request: ChatRequest):
    ...
    try:
        ...
        for msg in request.messages:
            ...
            logging.info(f"Request {request_id} - Masked content: {masked_content}")
        ...
        logging.info(
            f"Request {request_id} - Original LLM response: {response.choices[0]['message']['content']}"
        )
        ...
        raise HTTPException(status_code=503, detail=f"Unable to connect to LLM API")
