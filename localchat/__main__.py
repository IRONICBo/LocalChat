...
def call_local_api(prompt, model):
    ...
    headers = {"Content-Type": "application/json"}  # Set the header to inform the server that we are sending JSON data in the request body
    ...
    start_time = time.time()  # This function returns the current time in seconds since the epoch, and it's used to measure the API response time
    ...
    end_time = time.time()  # This is the end time in seconds right after the API response is received
    response_time = end_time - start_time  # Calculate how long the API took to respond (in seconds) by subtracting start_time from end_time
    ...
    if response.status_code == 200:
        result = response.json()  # Parse the response from the API as JSON. This will contain information about the completion, tokens, etc.
        ...
        prompt_tokens = result['usage'].get('prompt_tokens', 0)  # Tokens used for the input prompt
        completion_tokens = result['usage'].get('completion_tokens', 0)  # Tokens used for the generated completion text
        total_tokens = result['usage'].get('total_tokens', 0)  # Total tokens used, which is the sum of prompt_tokens and completion_tokens
        ...
        return f"Error: {response.status_code}, {response.text}", 0, 0, 0, 0  # This helps with debugging if the API call fails
...
def get_memory_usage():
    process = psutil.Process()  # Get the current process (the script itself) using psutil
    mem_info = process.memory_info()  # Retrieve memory usage statistics for the current process
    return mem_info.rss / (1024 * 1024)  # Return the memory usage in megabytes (RSS: Resident Set Size, which is the memory currently used by the process)
...
def chatgpt_clone(input, history, model, table_history):
    history = history or []  # If there's no existing chat history, initialize it as an empty list. This will store the conversation between the user and the chatbot
    table_history = table_history or []  # Similarly, if there's no existing table data, initialize it as an empty list. This will store performance metrics in a tabular format

    # Flatten the chat history into a list of strings, then append the new input prompt from the user
    s = list(sum(history, ()))  # Sum up the history to merge tuples into a flat list (flattening tuples into one list)
    s.append(input)  # Append the new user input to the conversation history
    inp = ' '.join(s)  # Join the list into a single string with spaces, forming the complete input prompt
    ...
    tokens_per_second = total_tokens / response_time if response_time > 0 else 0  # Only perform division if the response time is greater than zero

    # Append the new prompt and completion to the conversation history
    history.append((input, output))  # Store the conversation as tuples of (user_input, model_output) in the history
...
block = gr.Blocks()  # Blocks is the main container in Gradio that allows us to organize our interface components
...
with block:
    ...
    with gr.Row():
        chatbot = gr.Chatbot(label="Chatbot")  # The chatbot component shows the conversation between the user and the model
