...
def predict(
    query, ground_truth, docs, model, system, instruction, temperature, dataset
):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--noise_rate", type=float, default=0.0, help="rate of noisy passages"
    )
    parser.add_argument(
        "--api_key", type=str, default="api_key", help="api key of chatgpt"
    )
    parser.add_argument(
        "--plm", type=str, default="THUDM/chatglm-6b", help="name of plm"
    )
    parser.add_argument(
        "--url",
        type=str,
    ...
    parser.add_argument(
        "--correct_rate", type=float, default=0.0, help="rate of correct passages"
    )
    parser.add_argument(
        "--passage_num", type=int, default=5, help="number of external passages"
    )
    parser.add_argument(
        "--factchecking", type=bool, default=False, help="whether to fact checking"
    )
    ...
    if args.factchecking:
        prompt = yaml.load(
            open("config/instruction_fact.yaml", "r"), Loader=yaml.FullLoader
        )[args.dataset[:2]]
        resultpath = resultpath + "/fact"
    else:
        prompt = yaml.load(
            open("config/instruction.yaml", "r"), Loader=yaml.FullLoader
        )[args.dataset[:2]]
    ...
    with open(filename, "w") as f:
        for instance in tqdm.tqdm(instances):
            if (
                instance["id"] in useddata
                and instance["query"] == useddata[instance["id"]]["query"]
                and instance["answer"] == useddata[instance["id"]]["ans"]
            ):
    ...
    json.dump(
        scores,
        open(
            f"{resultpath}/prediction_{args.ollamamodel}_{args.dataset}_{modelname}_temp{temperature}_noise{noise_rate}_passage{passage_num}_correct{args.correct_rate}_result.json",
            "w",
        ),
        ensure_ascii=False,
        indent=4,
