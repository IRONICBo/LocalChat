...
# Define the file database model for rag raw files
...
# Define the session database model for storing chatbot dialogues
class Session(Base):
    __tablename__ = "dialogue"

    id = Column(Integer, primary_key=True, index=True)
    description = Column(String, nullable=False)  # Auto generated by the LLM
    llm = Column(String, nullable=False)  # Used LLM
    llm_settings = Column(String, nullable=False)  # Example, top_k xxx
    similarity_threshold = Column(Float)
    vector_similarity_weight = Column(Float)
    history = Column(
        String, nullable=False
    )  # For local senario, we just put all the history into one field
    created_at = Column(DateTime, default=func.now())
